...
public class Indg_HandleBatchPick implements YCDhandleShortageForBatchLineUE {
	...
	private void handleDamagedItems(YFSEnvironment env, YFCElement inputElem, String sBatchKey, YFCElement shipmentLines, String shortageReason, YFCDocument docInput) {
	    ...
	    if (cancellableShipments.size() > 0) {
	    	for (it = cancellableShipments.iterator(); it.hasNext();) {
	    		...
	    		boolean shipmentCancelled = checkShipmentForCancellation(env, shipmentKey, sBatchKey);
	    ...
	    for (Iterator<String> iterator = shipmentKeyList.iterator(); iterator.hasNext();) {
	    	...
	    	callChangeShipmentStatus(env, shKey, shipmentDetailsElem, sBatchKey, inputElem.getAttribute("AssignedToUserId"));
	    }
	}
	...
	private void callChangeShipmentForAllShipments(YFSEnvironment env, Map<String, YFCElement> changeShipmentInputMap, String shortageReason, YFCDocument docInput) {
