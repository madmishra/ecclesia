...
public class IndgCheckOutCart extends AbstractCustomApi {
	...
	private void invokeReservationAPI() {
		YFCDocument reserveOut = invokeYantraApi("reserveAvailableInventory",
				docRsrvInvInXml);
//		YFCDocument reserveOut = YFCDocument.getDocumentFor(new File("C:/Input/RAIOutput1.xml"));
		...
		for (String strStore : storeList) {
			...
			ArrayList<Element> alRsrv = SCXmlUtil.getElementsByAttribute(
					docReservOut.getDocumentElement(), "PromiseLines/PromiseLine",
					"ShipNode", strStore);
			...
			for (int i = 0; i < alRsrv.size(); i++) {
				...
				Element eleReservations = (Element) elePromiseLine.getElementsByTagName("Reservations").item(0);
				Element eleReservation = (Element) eleReservations.getElementsByTagName("Reservation").item(0);

				YFCElement eleLineItem = eleLineItems.createChild("LineItem");
				eleLineItem.setAttribute("Id",
						alRsrv.get(i).getAttribute("ItemID"));
				String strQtyToBeReserved = eleReservations.getAttribute(
						"QtyToBeReserved");
				String strReservedQty = eleReservations.getAttribute(
						"TotalReservedQty");
				eleLineItem.setAttribute("RequestedQuantity",
						strQtyToBeReserved);
				eleLineItem.setAttribute("ReservedQuantity", strReservedQty);
				eleLineItem.setAttribute("AvailableQuantity", eleReservations.getAttribute(
						"AvailableQty"));

				if (!cartAttributesPopulated) {
					eleCart.setAttribute("ReservationId", eleReservation.getAttribute("ReservationID"));
					eleCart.setAttribute("ReservationExpiryTime", eleReservation.getAttribute("ExpirationDate"));
	...
	private void prepareInputForReservation(YFCDocument inXml) {
		...
		if (nlLineItems.getLength() > 0) {
			...
			for (YFCElement eleLineItem : nlLineItems) {
				...
				elePromiseLine.setAttribute("ProductClass", getProperty("ProductClass", "GOOD"));
//				elePromiseLine.setAttribute("UnitOfMeasure","EACH");
//				elePromiseLine.setAttribute("ProductClass", "GOOD");
			}
//			System.out.println("The reserveAvailableInventory Input:" + docRsrvInvInXml);
			log.verbose("The reserveAvailableInventory Input:" + docRsrvInvInXml);
		}
	}
	...
	private void populateRequiredData(YFCDocument inXml) {
		...
		if (SCUtil.isVoid(skipCapacityChecks)) {
			...
			skipCapacityChecks = getProperty("SkipCapacityChecks",
					true);
		}
